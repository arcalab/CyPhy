...
\title{Calculus of Communicating Systemsss}
...
\begin{frame}{Sprinkles of linguistics}
...
\begin{frame}{The need for semantics in programming}
        ...
        How can one \alert{prove} that a program is \alert{correct} if its
...
\begin{frame}{Transition systems as semantic providers}
  ...
  \vfill
  We will use them to define the semantics of a simple (but \alert{powerful}
  !!) concurrent language \dots\
...
\begin{frame}{Calculus of communicating systems}
  ...
  $a. P$ \alert{communicates} via \alert{channel} $a$ and proceeds as $P$

  $\sum_{ i \in I} P_i$ non-deterministic choice between processes $P_i$

  $P \parallel Q$ \alert{parallel composition}
          between processes $P$ and $Q$

  $P \backslash L$ makes channels in $L$ private `outside' of $P$
\end{frame}
...
\begin{frame}{CCS and cyclic behaviour}
  We now add the construct \alert{$\mathtt{rec}\ X.\ P$} to the syntax of CCS
