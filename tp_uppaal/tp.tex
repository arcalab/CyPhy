...
\title{The curious case of an airport}
\author{Renato Neves}
 
\maketitle

Consider a private airport used by two planes, which can be either flying,
parked, landing, or taking off. The airport is a \emph{resource} shared by the
two planes, and the following requirements must be met:
\begin{enumerate}
  \item only one plane can use the runway at a time;
  ...
  \item if a plane is told to wait, we assume it will take between 5-7 time units to request again.
\end{enumerate}

\noindent
Suggest an \textsc{Uppaal} model for the planes and the controller. List 4 to 8
desired properties that the model should satisfy. Verify the properties via
\textsc{Uppaal}.

\smallskip
\noindent {\underline{The second, more exploratory stage}:} Extend your model to handle $n$ planes at
once and $n$ runways at once. Can you think of other useful features that the
airport should have? If so please discuss them and present the respective
models/properties.
...
\begin{mdframed}  
  ...
  ``\bash{cpc2526-N1-N2.zip}'', where \bash{N1} and \bash{N2} are your
  student numbers. The subject of the email should be ``\bash{cpc2526
    N1 + N2 first assignment}''.

\myparagraph{Deadline:} 16th April 2026 @ 23h59
\end{mdframed}

\bigskip
\begin{center}
        \href{https://docs.uppaal.org/}{\textsc{UPPAAL}'s reference manual} 
\end{center}
